 ...
 * codex-specific executor, the `/context` routes, and the startup banner.
 */
...
export async function createA2AServer(config: Required<AgentConfig>): Promise<ServerHandle> {
  const handle = await coreCreateA2AServer<Required<AgentConfig>>(config, (cfg) => new CodexExecutor(cfg), {
    protocolVersion: "1.0",
    registerRoutes: (app, executor) => {
      const codexExecutor = executor as CodexExecutor;

      // Context API
      app.get("/context", async (_req, res) => {
        try {
          const content = await codexExecutor.getContextContent();
          if (content === null) {
      ...
      app.post("/context/build", async (req, res) => {
        try {
          ...
          const response = await codexExecutor.buildContext(customPrompt);
          const content = await codexExecutor.getContextContent();
...
╚══════════════════════════════════════════════════════════════╝
  ...
  return {
    ...handle,
    executor: handle.executor as CodexExecutor,
    async shutdown() {
